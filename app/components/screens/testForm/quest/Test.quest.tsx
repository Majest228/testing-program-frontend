...
import { setSelected, setCurrentQuestion } from "@/app/store/test/test.slice";
...
const Quest = ({ id, prev, next }: any) => {
  ...
  const {
    ...
    error,
    isLoading,
  } = useQuery("test without right", () =>
    TestsService.getWithoutIsRight(Number(pathId))
  );
  const { data: title, isLoadingTitle } = useQuery("title by id", () =>
    TestsService.getTitle(Number(pathId))
  );
      ...
      <div className={styles.TestForm__content__middle__content}>
        <div className={styles.TestForm__content__middle__content__quest}>
          ...
          >
            ...
            >
              <p>
                {isLoading ? "" : title?.data[currentQuestion - 1].question}
              </p>
            </div>
          </div>
        </div>
